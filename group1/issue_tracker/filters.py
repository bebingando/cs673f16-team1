...
class FilterIssueQueryset(object):
    ...
    def convert_date_min(self, item):
        return timezone.make_aware(datetime.datetime.combine(item, datetime.time.min), timezone.get_current_timezone())

    def convert_date_max(self, item):
        return timezone.make_aware(datetime.datetime.combine(item, datetime.time.max), timezone.get_current_timezone())

    def filter_submitted_date(self, query):
        if self.submitted_date:
            query &= Q(submitted_date__range=(self.convert_date_min(self.submitted_date),
    ...
    def filter_modified_date(self, query):
        if self.modified_date:
            query &= Q(modified_date__range=(self.convert_date_min(self.modified_date),
    ...
    def filter_closed_date(self, query):
        if self.closed_date:
            query &= Q(closed_date__range=(self.convert_date_min(self.closed_date),
...
def filter_issue_results(data):
    ...
    # Based on parameters from the form, find the appropriate filter method and call it against the current queryset.
