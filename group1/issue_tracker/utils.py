...
def create_user(first, last, username, out_handle=None):
    ...
    try:
        user = User.objects.create_user(
            ...
            password='%spw' % username, **{'first_name': first, 'last_name': last}
        )
        user.save()
    except IntegrityError:
        if out_handle:
            out_handle.write('User already exists, skipping creation: %s' % username)
...
def get_random_date():
    return datetime.datetime.now() + datetime.timedelta(days=random.randint(-365, 365))
...
def create_issues(number_of_issues, out_handle=None):
    ...
    Args:
      number_of_issues: The number of issues to be created.
      out_handle: The output handler for printing.
    """
    ...
    for _ in xrange(number_of_issues):
        ...
        status = it_models.STATUSES[random.randint(0, len(it_models.STATUSES) - 1)][0]
        p = project_api.get_project(project_ids[random.randint(0, len(project_ids) - 1)])
        # Limit the users to the developers on the project
        user_ids = [user_ass.user.pk for user_ass in project_api.get_project_users(p.id)]
        if status != 'new':
            assignee = User.objects.get(pk=user_ids[random.randint(0, len(user_ids) - 1)])
        else:
            assignee = None
        if random.randint(0, 1):
            verifier = None
        else:
            verifier = User.objects.get(pk=user_ids[random.randint(0, len(user_ids) - 1)])

        issue = it_models.Issue.objects.create(
            title=TITLES[random.randint(0, title_count)],
            description=DESCRIPTIONS[random.randint(0, description_count)],
            issue_type=it_models.TYPES[random.randint(0, len(it_models.TYPES) - 1)][0],
            status=status,
            priority=it_models.PRIORITIES[random.randint(0, len(it_models.PRIORITIES) - 1)][0],
            project=p,
...
def create_projects(number_of_projects, out_handle=None):
    ...
    # This extra work for user ids is necessary due to the fact that the  database might have been wiped, but the origin
    # pk for the user still has been claimed, so we must gather the current pk list to apply them to create issues.
    ...
    for i in xrange(number_of_projects):
        if out_handle:
            ...
            out_handle.flush()
        fields = {
            'title': '%s: %d' % (PROJECTS[random.randint(0, project_count)][0], i),
            'description': PROJECTS[random.randint(0, project_count)][1]
        }
        excluded_users = [_get_random_user(user_ids)]
        p = project_api.create_project(user=excluded_users[0], fields=fields)
        ...
        for _ in xrange(random.randint(1, 5)):
            ...
            project_api.add_user_to_project(p.id, u.username, user_association_model.ROLE_DEVELOPER)
